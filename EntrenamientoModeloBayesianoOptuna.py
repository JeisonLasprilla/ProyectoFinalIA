...
class ModelTrainer:
    ...
    def save_evaluation_metrics(self, y_test, y_pred, class_names):
        ...
        metrics_df.to_csv(self.results_dir / 'classification_metrics.csv')
        logging.info(f"Evaluation metrics saved to {self.results_dir / 'classification_metrics.csv'}")

    def plot_metrics_by_class(self, y_test, y_pred, class_names):
        """Genera un gráfico de barras con precision, recall y F1-score por clase."""
        metrics_dict = classification_report(y_test, y_pred, target_names=class_names, output_dict=True)
        
        # Crear DataFrame para las métricas por clase
        metrics_data = {
            'Precision': [],
            'Recall': [],
            'F1-score': []
        ...
        plt.tight_layout()
        plt.savefig(self.results_dir / 'metrics_by_class.png')
        plt.close()

    def plot_confusion_matrices(self, y_test, y_pred, class_names):
        """Genera matrices de confusión normalizadas y sin normalizar."""
        ...
        plt.tight_layout()
        plt.savefig(self.results_dir / 'confusion_matrices.png')
        plt.close()
    ...
    def train_model(self, force_optimization: bool = True):
        ...
        # Obtener nombres de clases originales
    ...
    def plot_feature_importance(self):
        """Genera gráfico de importancia de características para Random Forest."""
